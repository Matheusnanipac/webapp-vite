...
export default function Business() {
  const [data, setData] = useState<RestaurantResponse | undefined>();
  ...
  const handleNavigation = (product: string) => {
    if (!product) return;
    navigate(`${params.businessId}/${product}`);
  };

  const handleValidateUrl = async () => {
    const resp = await getRestaurantSearchUrl(params?.businessId as string);
    if (resp.id) setData(resp);
    else redirect("/");
  };

  useEffect(() => {
    if (params && params.businessId) handleValidateUrl();
          ...
          <div className="container">
            ...
            <div className="flex mt-4 mb-6 text-left lg:mt-8 lg:mb-12">
              <p className="text-ellipsis overflow-x-hidden line-clamp-2 lg:text-center">
                ...
                lacus, nec aliquet nunc nisl nec justo lorem ipsum dolor sit
                amet, consectetur adipiscing elit. Sed euismod, nisi ac
                tincidunt aliquam, mauris nunc ultrices lacus, nec aliquet nunc
                nisl nec justo.
              </p>
            </div>
            <MenuCategory />
